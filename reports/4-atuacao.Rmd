---
title: "Atuação na tramitação das proposições"
output:
  html_document:
    theme: paper
    css: styles.css
---

```{r setup, include=FALSE, echo=FALSE}
knitr::opts_chunk$set(
  echo = FALSE,
  # message = FALSE,
  # warning = FALSE,
  # fig.cap = '',
  # fig.align = 'center',
  fig.width = 6,
  fig.height = 5
)

Sys.setenv(LANGUAGE="pt-br")
Sys.setlocale("LC_TIME", "pt_BR")
options(scipen = 999)
```

```{r warning=FALSE, message=FALSE}
library(tidyverse)
library(reactable)
library(ggchicklet)
library(ggbeeswarm)
library(hrbrthemes)
theme_set(theme_ipsum_rc())

source(here::here("code/read_ready.R"))
```

TO DO incluir parlamentares com zero ações

```{r}
proposicoes = read_proposicoes()
detalhes_atuacao = read_atuacao()
resumo_atuacao = detalhes_atuacao %>%
  rename(nome_autor = nome_eleitoral,
         tipo_generico = tipo_acao) %>% 
  group_by(casa,
           nome_autor,
           partido,
           uf,
           governismo,
           peso_politico,
           tipo_generico) %>%
  summarise(acoes = n_distinct(id_principal, casa, id_documento), 
            .groups = "drop")
```

Considerando e ignorando proposições apensadas

```{r}
resumo_atuacao %>%
  filter(!is.na(nome_autor), tipo_generico != "Outros") %>% 
  group_by(nome_autor) %>% 
  mutate(acoes_total = sum(acoes)) %>% 
  ungroup() %>% 
  # filter((casa == "camara" & acoes_total > 25) | 
  #          (casa == "senado" & acoes_total > 10)) %>%
  filter(acoes_total > 100) %>% 
  group_by(casa) %>% 
  ggplot(aes(
    x = reorder(nome_autor, acoes_total, sum),
    y = acoes,
    fill = tipo_generico
  )) +
  facet_grid(casa ~ ., scales = "free") +
  geom_chicklet() +
  coord_flip() +
  scale_fill_brewer(palette = "Accent") +
  labs(
    x = "",
    y = "Ações",
    fill = "Tipo da ação",
    title = "Parlamentares mais atuantes",
    subtitle = "Com ao menos 100 ações"
  )
```

```{r}
resumo_atuacao %>% 
  group_by(casa,
           nome_autor,
           partido,
           uf,
           governismo,
           peso_politico) %>% 
  summarise(acoes = sum(acoes)) %>% 
  ggplot(aes(x = governismo, y = acoes)) + 
  geom_point()
  
```


