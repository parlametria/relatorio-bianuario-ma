...
participacao = detalhes_autorias %>%
  filter(!is.na(classificacao_ambientalismo), !(sigla_tipo %in% c("PEC", "MPV"))) %>% 
...
participacao %>% 
  filter(autores > 1) %>% 
  ggplot(aes(x = classificacao_ambientalismo, y = autores, color = classificacao_ambientalismo)) + 
...
participacao %>%
  filter(partidos > 1) %>%
  ggplot(
    aes(x = classificacao_ambientalismo, y = partidos, color = classificacao_ambientalismo)
  ) +
  geom_quasirandom(show.legend = F) +
  escala_cor_sinais +
  coord_flip() +
